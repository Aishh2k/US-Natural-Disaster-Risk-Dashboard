...
function initializeSlider() {
    ...
    slider.addEventListener('input', (e) => {
        ...
        if (currentView === 'historical') {
            ...
            if (selectedState) updateSidebar(selectedState);
        }
    });
    yearDisplay.textContent = currentYear;
}
...
function showDefaultSafetyTips() {
    const safetyListElem = document.getElementById('safety-list');
    if (!safetyListElem) return;

    safetyListElem.innerHTML = '';
    HAZARD_SUGGESTIONS["_default"].forEach(msg => {
        const li = document.createElement('li');
        li.textContent = msg;
        safetyListElem.appendChild(li);
    });
...
function forceChatbotButtonVisible() {
...
function resetHighlight(e) {
    ...
    showDefaultSafetyTips();
}

function selectState(e, feature) {
    const stateName = feature.properties.name;
    const wasSelected = selectedState === stateName;
